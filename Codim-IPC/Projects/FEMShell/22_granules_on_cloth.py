...
if __name__ == "__main__":
    ...
    sim.add_particle_3D(
        ...
        Vector3i(N, H, N),
        Vector3d(5e-4, 5e-4, 5e-4),
        Vector3d(0, step * (H - 1) / 2 + step, 0),
        Vector3d(0, 0, 0),
        Vector3d(1, 0, 0),
        0,
        Vector3d(1, 1, 1),
    ...
    sim.add_shell_with_scale_3D(
        "input/square21.obj", Vector3d(0, -0.3, 0), Vector3d(8, 8, 8), Vector3d(0, 0, 0), Vector3d(1, 0, 0), 0
    )
    sim.set_DBC(
        Vector3d(-0.1, -0.1, -0.1),
    ...
    if algI == 0:
        ...
        # iso, no strain limit
        sim.initialize(
            sim.cloth_density_iso[clothI], membEMult, sim.cloth_nubase_iso[clothI], sim.cloth_thickness_iso[clothI], 0
        )
        sim.bendingStiffMult = bendEMult / membEMult
