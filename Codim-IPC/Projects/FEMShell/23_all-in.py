...
if __name__ == "__main__":
    ...
    for i in range(N):
        ...
        sim.make_and_add_rod_3D(
            1,
            int(rodSize),
            Vector3d(0, 0, -0.5 + step / 2 + step * i),
            Vector3d(0, 0, 0),
            Vector3d(1, 0, 0),
            0,
            Vector3d(1, 1, 1),
        ...
        if up == 1:
            sim.make_and_add_rod_3D(
                1,
                int(rodSize),
                Vector3d(-0.5 + step / 2 + step * i, 1e-3, 0),
                Vector3d(0, 0, 0),
                Vector3d(0, 1, 0),
                90,
                Vector3d(1, 1, 1),
    ...
    # floor
    sim.add_shell_with_scale_3D(
        "input/square21.obj", Vector3d(0, -0.5, 0), Vector3d(8, 8, 8), Vector3d(0, 0, 0), Vector3d(1, 0, 0), 0
    )
    sim.set_DBC(
        Vector3d(-0.1, -0.1, -0.1),
        Vector3d(1.1, 1e-3, 1.1),
        Vector3d(0, 0, 0),
        Vector3d(0, 0, 0),
        Vector3d(1, 0, 0),
        0,
    ...
    H = 100
    step = 4e-3
    counter = sim.add_particle_3D(
        Vector3d(step * (N - 1), step * (H - 1), step * (N - 1)),
        Vector3i(N, H, N),
        Vector3d(1.25e-3, 1.25e-3, 1.25e-3),
        Vector3d(0, step * (H - 1) / 2 + 1.01, -0.1),
        Vector3d(0, 0, 0),
        Vector3d(1, 0, 0),
        0,
        Vector3d(1, 1, 1),
    ...
    # elastic body
    sim.add_object_3D(
        "../FEM/input/Armadillo13K.vtk",
        Vector3d(0, 0.8, 0),
        Vector3d(0, 0, 0),
        Vector3d(1, 0, 0),
        -90,
        Vector3d(0.003, 0.003, 0.003),
    )

    # cloth
    counter = sim.add_shell_with_scale_3D(
