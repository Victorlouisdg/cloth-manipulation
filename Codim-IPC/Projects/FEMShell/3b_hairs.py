    ...
    for j in range(N):
        ...
        for i in range(iMax):
            sim.make_and_add_rod_3D(
                0.2,
                int(size),
                Vector3d(-xWidth / 2 + step * i, -width / 2 + step * j, 0),
                Vector3d(0, 0, 0),
                Vector3d(0, 1, 0),
                90,
                Vector3d(1, 1, 1),
    ...
    for j in range(N):
        ...
        for i in range(iMax):
            ...
            sim.make_and_add_rod_3D(
                0.2 - dl,
                int(size),
                Vector3d(0.053 + dl / 2, width + step - xWidth / 2 + step * i, -width / 2 + step * j),
                Vector3d(0, 0, 0),
                Vector3d(0, 1, 0),
                0,
                Vector3d(1, 1, 1),
            ...
            rodCounter = rodCounter + 1
    sim.set_DBC(
        Vector3d(1 - 1.0 / (int(size) - 2), -0.1, -0.1),
        Vector3d(1.1, 1.1, 1.1),
        Vector3d(0, 0, 0),
        Vector3d(0, 0, 0),
        Vector3d(1, 0, 0),
