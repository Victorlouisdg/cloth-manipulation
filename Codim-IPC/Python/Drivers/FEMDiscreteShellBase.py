...
class FEMDiscreteShellBase(SimulationBase):
    def __init__(self, precision, dim, output_folder):
        super().__init__(precision, dim, output_folder)
        ...
        self.cloth_Ebase_iso = [
            0.821e6,
            0.170e6,
            0.076e6,
            0.478e6,
            3e9,
        ]  # for memb: 0.1x, 0.2, 1x, 0.1x, 2e4 ~ 1e5, for bending: 5e4 ~ 1e6
        ...
        self.cloth_weftWarpMult = [
            Vector4d(15.557e6, 7.779e6, 25.004e6, 1.076e6),
        ...
        self.cloth_inextLimit = [
            Vector3d(0.14, 0.14, 0.063),
    ...
    def add_mannequin(self, filePath, translate, scale, rotCenter, rotAxis, rotDeg):
        meshCounter = FEM.DiscreteShell.Add_Shell(
            filePath,
            translate,
            scale,
            rotCenter,
    ...
    def initialize(self, p_density, E, nu, thickness, caseI):
        ...
        self.dHat2 = FEM.DiscreteShell.Initialize_Shell_Hinge_EIPC(
            p_density,
            E,
            nu,
            thickness,
    ...
    def reinitialize_argus(self, filePath):
        FEM.DiscreteShell.Update_Material_With_Tex_Shell(
            filePath,
            0,
            0,
            self.shell_density,
    ...
    def adjust_material(self, meshCounter, densityMult, YoungsMult):
        FEM.DiscreteShell.Adjust_Material(
            meshCounter,
            densityMult,
            YoungsMult,
            self.nodeAttr,
    ...
    def initialize_rod(self, p_density, E, bendStiffMult, thickness):
        self.dHat2 = FEM.DiscreteShell.Initialize_Discrete_Rod(
            self.X,
            self.rod,
            p_density,
            E,
    ...
    def initialize_particle(self, p_density, E, thickness):
        FEM.DiscreteShell.Initialize_Discrete_Particle(
            self.X,
    ...
    def initialize_OIPC(self, thickness, offset, stiffMult=1):
        self.dHat2 = FEM.DiscreteShell.Initialize_OIPC(
            0.0, 0.0, thickness, 0.0, self.massMatrix, self.kappa, stiffMult
        )
        self.elasticIPC = False
    ...
    def advance_one_time_step(self, dt):
        ...
        if self.normalFlowMag != 0:
            ...
            FEM.DiscreteShell.Update_Normal_Flow_Neumann(
                self.X, self.Elem, self.massMatrix, self.normalFlowMag, self.bodyForce
            )
        ...
        if self.t < self.MDBC_tmax and self.t > self.MDBC_tmin:
            if self.seqDBC < 0:
                ...
                FEM.Load_Dirichlet(
                    self.seqDBCPath + "/shell" + str(self.curFrameNum) + ".obj",
                    self.seqDBC,
                    Vector3d(0, -0.75, 0),
                    self.DBC,
                )
        ...
        if (
            self.DBCPopBackCounter < self.DBCPopBackAmt
            and self.t < self.DBCPopBackTEnd
            and self.t > self.DBCPopBackTStart
        ):
        ...
        if self.lv_fn >= 0:
            ...
            MeshIO.Load_Velocity_X0(
                self.seqDBCPath, self.lv_fn, self.dt, self.X, self.nodeAttr
            )  # calc dx based on x_cur
            FEM.Load_Dirichlet(
                self.seqDBCPath + "/" + str(self.lv_fn) + ".obj",
                0,
                Vector3d(0, 0, 0),
                self.DBC,
            )
        ...
        if (
            ...
            if (self.scaleXMultStep > 1 and self.scaleX > self.scaleXTarget) or (
                self.scaleXMultStep < 1 and self.scaleX < self.scaleXTarget
            ):
                self.scaleX = self.scaleXTarget
            if (self.scaleYMultStep > 1 and self.scaleY > self.scaleYTarget) or (
                self.scaleYMultStep < 1 and self.scaleY < self.scaleYTarget
            ):
                self.scaleY = self.scaleYTarget
            if (self.scaleZMultStep > 1 and self.scaleZ > self.scaleZTarget) or (
    ...
    def write(self, frame_idx):
        shell_path = os.path.join(self.output_folder, f"shell{frame_idx}.obj")
        seg_path = os.path.join(self.output_folder, f"seg{frame_idx}.obj")
        rod_path = os.path.join(self.output_folder, f"rod{frame_idx}.obj")
        vol_path = os.path.join(self.output_folder, f"vol{frame_idx}.obj")

        MeshIO.Write_TriMesh_Obj(self.X, self.Elem, shell_path)
        if self.outputSeg:
            MeshIO.Write_SegMesh_Obj(self.X, self.segs, seg_path)
        if self.outputRod:
