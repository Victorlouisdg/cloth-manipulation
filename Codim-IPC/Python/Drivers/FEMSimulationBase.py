...
class FEMSimulationBase(SimulationBase):
    def __init__(self, precision, dim, model="FCR"):
        ...
        self.elasticity = (
            FIXED_COROTATED_2.Create() if self.dim == 2 else FIXED_COROTATED_3.Create()
    ...
    def initialize_added_objects(self, velocity, p_density, E, nu):
        ...
        if self.enableFracture:
            FEM.Fracture.Initialize_Fracture(
                self.X0,
                self.Elem,
                self.debrisAmt,
                self.fractureRatio2,
                self.strengthenFactor,
                self.edge_dupV,
                self.isFracture_edge,
                self.incTriV_edge,
                self.incTriRestDist2_edge,
            )
        if self.EIPC:
            FEM.TimeStepper.ImplicitEuler.Initialize_Elastic_IPC(
                self.X0, self.Elem, self.dt, E, nu, self.dHat2, self.kappa
            )
    ...
    def advance_one_time_step(self, dt):
        ...
        if self.useNewton:
            if self.EIPC:
                self.PNIterCount += FEM.TimeStepper.ImplicitEuler.Advance_One_Step_EIPC(
                    self.Elem,
                    self.DBC,
                    self.gravity,
                    self.dt,
                ...
                self.PNIterCount += FEM.TimeStepper.ImplicitEuler.Advance_One_Step(
                    self.Elem,
                    self.DBC,
                    self.gravity,
                    self.dt,
            ...
            self.PNIterCount += FEM.TimeStepper.ImplicitEuler.Advance_One_Step_SU(
                self.Elem,
                self.DBC,
                self.gravity,
                self.dt,
        ...
        if self.enableFracture:
            if FEM.Fracture.Edge_Fracture(
                ...
                FEM.Fracture.Node_Fracture(self.edge_dupV, self.isFracture_edge, self.X, self.Elem, self.finalV2old)
                print("node fractured")
                FEM.Fracture.Update_Fracture(
                    self.Elem,
                    self.rho0,
                    self.finalV2old,
                    self.X0,
    ...
    def write(self, frame_idx):
        MeshIO.Write_TriMesh_Obj(
            self.X, self.Elem, self.output_folder + str(frame_idx) + ".obj"
        ) if self.dim == 2 else MeshIO.Write_Surface_TriMesh_Obj(
            self.X, self.TriVI2TetVI, self.Tri, self.output_folder + str(frame_idx) + ".obj"
        )
